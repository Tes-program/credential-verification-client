...
const ManageStudents: React.FC = () => {
  ...
  const filteredStudents = React.useMemo(() => {
    return students.filter(student => {
      ...
      const matchesProgram = 
        selectedProgram === 'All Programs' || student.program === selectedProgram;
  ...
  const handleIssueCredential = () => {
    if (selectedStudents.length === 0) {
      toast({
        ...
        description: "Please select at least one student to issue a credential.",
        status: "warning",
        duration: 3000,
      });
      return;
      ...
      <Container maxW="container.lg" py={8}>
        <Alert status="error" borderRadius="md">
          ...
          <Box>
            ...
            <Text>Only institution accounts can access student management.</Text>
          </Box>
        </Alert>
  ...
  return (
    <Container maxW="container.xl" py={8}>
              ...
              {isLoading ? (
                ...
                filteredStudents.map(student => (
                  ...
                  >
                    ...
                    <Td onClick={() => viewStudentDetails(student)}>
                      <Badge 
                        colorScheme={student.credentialsCount > 0 ? "blue" : "gray"}
                        borderRadius="full"
                        px={2}
                      >
                    ...
                    <Td onClick={() => viewStudentDetails(student)}>
                      <Badge 
                        colorScheme={
                          ...
                          student.status === 'graduated' ? 'purple' : 'gray'
                        }
                      >
                        {student.status}
                      </Badge>
                ...
                <Tr>
                  <Td colSpan={7} textAlign="center" py={6}>
                    ...
                    <Text fontSize="sm" color="gray.500">
                      {searchTerm || selectedProgram !== 'All Programs' || statusFilter !== 'all'
      ...
      {!isLoading && filteredStudents.length > 0 && (
        ...
        >
          ...
          <HStack>
            ...
            <Text fontWeight="bold">{students.filter(s => s.status === 'active').length}</Text>
          </HStack>
          
          <HStack>
            <Text color="gray.500">Graduated:</Text>
            <Text fontWeight="bold">{students.filter(s => s.status === 'graduated').length}</Text>
          </HStack>
      ...
      {/* Add Student Modal */}
      <Modal isOpen={addStudentModal.isOpen} onClose={addStudentModal.onClose} size="xl">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>
            <Flex align="center">
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              ...
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                <FormControl isRequired>
                  ...
                  <Select placeholder="Select program">
                    {programOptions.filter(p => p !== 'All Programs').map(program => (
                      <option key={program} value={program}>{program}</option>
                    ))}
                  </Select>
                </FormControl>
              ...
              <Alert status="info" borderRadius="md">
                <AlertIcon />
                The student will receive an email invitation to create their account on the platform.
              </Alert>
            </VStack>
          </ModalBody>
                  ...
                  <Box flex="1">
                    ...
                    <Badge 
                      colorScheme={
                        ...
                        selectedStudent.status === 'graduated' ? 'purple' : 'gray'
                      }
                      mt={1}
                    >
                  ...
                  <TabPanels>
                    ...
                    <TabPanel px={0}>
                      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6}>
                        <VStack align="start" spacing={3}>
                          ...
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Program</Text>
                            <Text>{selectedStudent.program}</Text>
                          </Box>
                          
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Enrollment Year</Text>
                            <Text>{selectedStudent.enrollmentYear}</Text>
                          </Box>
                          
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Expected Graduation</Text>
                        ...
                        <VStack align="start" spacing={3}>
                          ...
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Last Activity</Text>
                            <Text>{new Date(selectedStudent.lastActivity).toLocaleDateString()}</Text>
                          </Box>
                          
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Credentials Issued</Text>
                            <Text>{selectedStudent.credentialsCount}</Text>
                          </Box>
                          
                          <Box width="100%">
                            <Text fontWeight="medium" fontSize="sm" color="gray.500">Internal ID</Text>
                            <Text fontSize="sm" fontFamily="mono">{selectedStudent.id}</Text>
                          </Box>
                        </VStack>
                      </SimpleGrid>
                      
                      <Flex justify="flex-end" mt={6} gap={3}>
                        <Button leftIcon={<Icon as={FaEdit} />} variant="outline">
                          Edit Details
                        </Button>
                        <Button 
                          leftIcon={<Icon as={FaCertificate} />} 
                    ...
                    <TabPanel px={0}>
                      {selectedStudent.credentialsCount > 0 ? (
                        <VStack align="stretch" spacing={4}>
                          {[...Array(selectedStudent.credentialsCount)].map((_, i) => (
                            <Box 
                              key={i} 
                              p={4} 
                              borderWidth="1px" 
                              ...
                              <Flex direction={{ base: 'column', md: 'row' }} justify="space-between">
                                <Box>
                                  <Flex align="center" mb={2}>
                                    ...
                                    <Heading size="sm">
                                      {i === 0 ? 'Bachelor of Science in Information Technology' : 
                                      i === 1 ? 'Certificate in Web Development' : 
                                      'Award for Academic Excellence'}
                                    </Heading>
                                  </Flex>
                                  <Text fontSize="sm" color="gray.600">
                                    Issued on: {new Date(Date.now() - i * 30 * 24 * 60 * 60 * 1000).toLocaleDateString()}
                                  </Text>
                                </Box>
                        ...
                        <Box 
                          ...
                          borderStyle="dashed"
                        >
                          <Icon as={FaCertificate} boxSize={10} color="gray.400" mb={3} />
                          <Heading size="md" mb={2}>No Credentials Issued</Heading>
                    ...
                    <TabPanel px={0}>
                      <VStack align="stretch" spacing={4}>
                        ...
                        <Box p={4} borderWidth="1px" borderRadius="md">
                          <Flex align="center">
                            <Icon as={FaUserGraduate} color="purple.500" mr={3} />
                            <Box>
                              <Text>Account created</Text>
                              <Text fontSize="sm" color="gray.500">
                                {new Date(Date.now() - 120 * 24 * 60 * 60 * 1000).toLocaleString()}
                              </Text>
                            </Box>
                          </Flex>
                        </Box>
          ...
          <ModalBody>
            <Alert status="info" mb={4} borderRadius="md">
              ...
              <Box>
                ...
                <AlertDescription>
                  We've detected a CSV file with student records. Would you like to proceed with importing?
                </AlertDescription>
              </Box>
            </Alert>
            ...
            <VStack align="stretch" spacing={3}>
              <Flex justify="space-between">
                ...
                <Badge colorScheme="blue" px={2}>24</Badge>
              </Flex>
              
              <Flex justify="space-between">
                <Text>New students to add:</Text>
                <Badge colorScheme="green" px={2}>20</Badge>
              </Flex>
              
              <Flex justify="space-between">
                <Text>Existing records to update:</Text>
                <Badge colorScheme="orange" px={2}>4</Badge>
              </Flex>
            </VStack>
          </ModalBody>
      ...
      <Modal isOpen={issueCredentialModal.isOpen} onClose={issueCredentialModal.onClose}>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody>
            <VStack align="stretch" spacing={4}>
              <Text>
                You are about to issue credentials to {selectedStudents.length} selected students.
              </Text>
                ...
                {selectedStudents.slice(0, 5).map(id => {
                  ...
                  return student ? (
                    <Tag key={id} colorScheme="blue" borderRadius="full">
                      <TagLabel>{student.firstName} {student.lastName}</TagLabel>
                    </Tag>
                  ) : null;
                })}
              ...
              <Alert status="info" borderRadius="md">
                <AlertIcon />
                Would you like to issue individual credentials or use a batch process?
              </Alert>
            </VStack>
          </ModalBody>
          
          <ModalFooter>
            <Button variant="outline" mr={3} onClick={issueCredentialModal.onClose}>
              Cancel
            </Button>
            <Button 
              as={RouterLink}
